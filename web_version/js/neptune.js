...
class GameState {
    constructor() {
        this.totime = 0; // total cumulative time 
        this.breed = 120; // breeder-reactor cells
        this.futot = 3000; // fuel cells
        this.distance = 0; // distance travelled
        this.efail = 0; // engine failure percentage
        this.fuseg = 0; // amount of fuel to use this segment
        this.rate = 0; // rate of speed (last seg)
        this.time = 0; // time spent (last seg)
        this.ubreed = 0; // used breeder cells (last seg)
        this.fubr = 0; // fuel used per breeder cell (last seg)
        this.fudcy = 0; // how much fuel decays (last seg)
    }
}
...
function fmt_days(t) {
    ...
    if (years > 0)
        result.push(bca.plural(years, 'year'));
    if (months > 0)
        result.push(bca.plural(months, 'month'));
    if (days > 0)
        result.push(bca.plural(days, 'day'));
    return result.join(', ');
}

function printConditions(ui, gs) {
    ...
    if (gs.seg.stage > 1) {
        ...
        ui.print(`Planned time for this total distance: ${fmt_days(0.81 * gs.distance)}<br>
                  ...
                  You used ${bca.plural(gs.ubreed,'cell')} which produced ${bca.plural(gs.fubr,'pound')} of fuel each.`);
        if (gs.fudcy > 0) {
...
async function tradingInterface(ui, gs) {
...
async function enginePower(ui, gs) {
    ...
    const cell_updater = function (fuel_val) {
